...
BEGIN
    ...
    SELECT COALESCE(SUM(
        CASE
            ...
            WHEN event_type = 'SHOT' THEN
                CASE 
                    WHEN metadata->>'result' = 'GOAL' THEN 1.0
                    WHEN metadata->>'result' = 'SAVED' THEN 0.1
                    WHEN metadata->>'result' = 'WOODWORK' THEN 0.1
                    WHEN metadata->>'result' = 'OFF_TARGET' THEN -0.1
                    WHEN metadata->>'result' = 'BLOCKED' THEN -0.05
                    ELSE 0
                END
            ...
            WHEN event_type::text = 'GOAL' THEN 1.0

            -- PASS
            WHEN event_type = 'PASS' THEN
                CASE
                    WHEN metadata->>'result' = 'COMPLETED' THEN 0.02
                        + CASE WHEN metadata->>'qualifier' = 'KEY_PASS' THEN 0.3 ELSE 0 END
                        + CASE WHEN metadata->>'qualifier' = 'ASSIST' THEN 0.8 ELSE 0 END
                        + CASE WHEN metadata->>'distance' = 'LONG' THEN 0.03 ELSE 0 END
                        + CASE WHEN metadata->>'type' = 'THROUGH_BALL' THEN 0.1 ELSE 0 END
                        + CASE WHEN y IS NOT NULL AND ((v_is_home AND y < 33) OR (NOT v_is_home AND y > 67)) THEN 0.03 ELSE 0 END -- Final Third bonus
            ...
            WHEN event_type = 'DRIBBLE' THEN
                CASE
                    ...
                    WHEN metadata->>'result' = 'UNSUCCESSFUL' THEN -0.1
                    ELSE -0.1 -- Catch any missing result as unsuccessful just in case
                END

            -- CROSS
            WHEN event_type = 'CROSS' THEN
                CASE
                    WHEN metadata->>'result' = 'COMPLETED' THEN 0.1
            ...
            WHEN event_type = 'TACKLE' THEN
                CASE
                    WHEN metadata->>'result' = 'WON_RETAINED' THEN 0.15
                    ...
                    WHEN metadata->>'result' = 'WON_LOOSE' THEN 0.1
                        + CASE WHEN metadata->>'type' = 'SLIDING' THEN 0.05 ELSE 0 END
                        + CASE WHEN y IS NOT NULL AND ((v_is_home AND y > 67) OR (NOT v_is_home AND y < 33)) THEN 0.1 ELSE 0 END
                    WHEN metadata->>'result' = 'LOST' THEN -0.1
                    WHEN metadata->>'result' = 'FOUL' THEN -0.2
                    ELSE 0
                END
            ...
            WHEN event_type IN ('BALL_RECOVERY', 'INTERCEPTION', 'CLEARANCE', 'BLOCK') THEN
                CASE
                    WHEN event_type = 'INTERCEPTION' OR metadata->>'type' = 'INTERCEPTION' THEN 0.1
                    WHEN event_type = 'CLEARANCE' OR metadata->>'type' = 'CLEARANCE' THEN 0.05
                        + CASE WHEN y IS NOT NULL AND ((v_is_home AND y > 83) OR (NOT v_is_home AND y < 17)) THEN 0.1 ELSE 0 END -- Penalty area clearance
                    WHEN event_type = 'BLOCK' OR metadata->>'type' = 'BLOCK' THEN 0.1
                    WHEN event_type = 'BALL_RECOVERY' OR metadata->>'type' = 'RECOVERY' THEN 0.03
            ...
            WHEN event_type = 'AERIAL_DUEL' THEN
                CASE WHEN metadata->>'result' = 'WON' THEN 0.05
            ...
            WHEN event_type = 'SAVE' THEN
                CASE
                    WHEN metadata->>'type' = 'CATCH' THEN 0.15
                    WHEN metadata->>'type' = 'PARRY_SAFE' THEN 0.1
                    WHEN metadata->>'type' = 'PARRY_DANGER' THEN 0.0
                    ELSE 0
                END
    ...
    SELECT COALESCE(SUM(
        CASE
            WHEN event_type::text = 'FOUL' THEN -0.2
