  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 p-6 border border-blue-200 rounded">
        <h2 className="text-xl font-bold text-blue-900 mb-4">Schedule Management</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button onClick={() => router.push(`/admin/events/${eventId}/slots`)} className="block text-left border rounded p-4 hover:shadow-md transition bg-white border-blue-300">
            <h3 className="font-bold text-blue-700">1. Slot Configurator</h3>
            <p className="text-sm text-gray-600 mt-1">Define tournament slot timings before scheduling begins.</p>
          </button>
          
          <button onClick={() => router.push(`/admin/events/${eventId}/scheduling-live`)} className="block text-left border rounded p-4 hover:shadow-md transition bg-white border-blue-300">
            <h3 className="font-bold text-red-600">2. Live Scheduling Command Center</h3>
            <p className="text-sm text-gray-600 mt-1">Assign fixtures and broadcast live schedules.</p>
          </button>
          
          <button onClick={() => window.open(`/events/${event.slug || eventId}`, '_blank')} className="block text-left border rounded p-4 hover:shadow-md transition bg-white border-blue-300">
            <h3 className="font-bold text-emerald-600">3. View Public Tournament Page</h3>
            <p className="text-sm text-gray-600 mt-1">View the public site with standings and Match Centers.</p>
          </button>
        </div>
      </div>

      <form onSubmit={saveSettings} className="grid grid-cols-2 gap-6 bg-white p-6 border rounded">
        <div className="space-y-4">
          ...
          <div>
            ...
            <select className="w-full border p-2 rounded" value={event.venue_id || ''} onChange={e => setEvent({...event, venue_id: e.target.value})}>
          ...
          <div>
            ...
            <input type="datetime-local" className="w-full border p-2 rounded" value={event.registration_deadline?.substring(0,16) || ''} onChange={e => setEvent({...event, registration_deadline: new Date(e.target.value).toISOString()})} />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className="w-full border p-2 rounded" value={settings?.tournament_format || ''} onChange={e => setSettings({...settings, tournament_format: e.target.value})}>
                <option value="ROUND_ROBIN">ROUND_ROBIN</option>
                <option value="KNOCKOUT">KNOCKOUT</option>
              </select>
            </div>
            <div>
              <label className="block text-sm">Match Format / Timings</label>
              <div className="flex gap-2">
                <input type="number" placeholder="Half 1" className="w-full border p-2 rounded" value={settings?.first_half_minutes || ''} onChange={e => setSettings({...settings, first_half_minutes: parseInt(e.target.value)})} title="First Half Minutes" />
                <input type="number" placeholder="Half 2" className="w-full border p-2 rounded" value={settings?.second_half_minutes || ''} onChange={e => setSettings({...settings, second_half_minutes: parseInt(e.target.value)})} title="Second Half Minutes" />
                <input type="number" placeholder="Break" className="w-full border p-2 rounded" value={settings?.half_time_minutes || ''} onChange={e => setSettings({...settings, half_time_minutes: parseInt(e.target.value)})} title="Half Time Minutes" />
              </div>
            </div>
            <div><label className="block text-sm">Players on Field</label><input type="number" className="w-full border p-2 rounded" value={settings?.players_on_field || ''} onChange={e => setSettings({...settings, players_on_field: parseInt(e.target.value)})} /></div>
            <div><label className="block text-sm">Min Squad (Minimum to play)</label><input type="number" className="w-full border p-2 rounded" value={settings?.min_squad || ''} onChange={e => setSettings({...settings, min_squad: parseInt(e.target.value)})} /></div>
            <div>
              <label className="block text-sm">Max Substitutes Allowed</label>
              <input 
                type="number" 
            ...
            <div>
              <label className="block text-sm">Duplicate Jerseys</label>
              <input type="checkbox" checked={settings?.allow_duplicate_jersey_numbers || false} onChange={e => setSettings({...settings, allow_duplicate_jersey_numbers: e.target.checked})} />
            </div>
            <div>
              <label className="block text-sm">Stoppage Time / Injury Time UI</label>
              <label className="flex items-center gap-2 text-sm mt-2 cursor-pointer">
                <input type="checkbox" checked={settings?.injury_time_tracking || false} onChange={e => setSettings({...settings, injury_time_tracking: e.target.checked})} />
