...
export default function ChatWindow({ params }: { params: Promise<{ conversationId: string }> }) {
  ...
  return (
    <div className="flex flex-col h-[80vh] max-w-3xl mx-auto border rounded-lg bg-gray-50 overflow-hidden">
      <div className="bg-white p-4 border-b font-semibold shadow-sm">Conversation</div>
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((m) => {
          const isMine = m.sender_id === userId;
          return (
            <div key={m.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] rounded-2xl px-4 py-2 ${isMine ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white border text-gray-800 rounded-bl-none'}`}>
                <p>{m.body}</p>
