...
interface FoulModalProps {
  ...
  onClose: () => void;
  onConfirm: (playerCommitting: MatchPlayer | null, playerReceiving: MatchPlayer | null, teamCommitting: 'home' | 'away' | null) => void;
}
      ...
      <div className="bg-white w-full max-w-md sm:rounded-xl rounded-t-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto p-4 flex-1 bg-white space-y-4">
          ...
          <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
            <h3 className="font-bold text-sm text-slate-800 mb-3">1. Team Committing Foul</h3>
          ...
          {team && (
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
              <div className="flex justify-between items-center mb-3">
                <h3 className="font-bold text-sm text-slate-800">2. Player Committing Foul</h3>
                ...
                playersLoading ? <div className="text-xs text-slate-400">Loading...</div> : (
                  <div className="flex overflow-x-auto pb-2 gap-2 snap-x">
                    {activePlayers.filter(p => !['SUBBED_OUT', 'SENT_OFF'].includes(p.participationStatus || '')).map(p => (
                      <button key={p.id} onClick={() => handleSelectCommitting(p)} className="snap-start flex-none flex flex-col items-center bg-white p-2 rounded w-16 hover:bg-slate-100 border border-slate-200">
          ...
          {playerCommitting && (
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
              <div className="flex justify-between items-center mb-3">
                <h3 className="font-bold text-sm text-slate-800">3. Player Fouled (Receiver)</h3>
                ...
                {playerReceiving === 'UNKNOWN' && <span className="text-xs bg-slate-200 text-slate-700 font-bold px-2 py-0.5 rounded">Skipped</span>}
              </div>
              {!playerReceiving && (
                playersLoading ? <div className="text-xs text-slate-400">Loading...</div> : (
                  <div className="flex flex-col gap-3">
                    <div className="flex overflow-x-auto pb-2 gap-2 snap-x">
                      {opposingPlayers.filter(p => !['SUBBED_OUT', 'SENT_OFF'].includes(p.participationStatus || '')).map(p => (
                        <button key={p.id} onClick={() => handleSelectReceiving(p)} className="snap-start flex-none flex flex-col items-center bg-white p-2 rounded w-16 hover:bg-slate-100 border border-slate-200">
                          <span className="w-8 h-8 flex items-center justify-center bg-slate-100 rounded text-slate-700 font-bold mb-1 shadow-sm text-sm">{p.jersey_number || '-'}</span>
                          <span className="text-[10px] font-medium text-center leading-tight line-clamp-2">{p.name}</span>
                        </button>
                      ))}
                    ...
                    <button 
                      ...
                      className="py-2 text-sm text-slate-600 font-medium rounded border border-slate-200 bg-white shadow-sm hover:bg-slate-50"
                    >
                      Skip / Unknown
                    </button>
                  </div>
        ...
        <div className="p-3 border-t bg-white shrink-0">
          <button 
            ...
            onClick={() => onConfirm(playerCommitting, playerReceiving === 'UNKNOWN' ? null : playerReceiving, team)} 
            className={`w-full py-3 font-bold rounded-lg shadow-sm ${(!playerCommitting || !playerReceiving) ? 'bg-slate-200 text-slate-400' : 'bg-slate-800 text-white hover:bg-slate-900'}`}
          >
            Log Foul
          </button>
        </div>
