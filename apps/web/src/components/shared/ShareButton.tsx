...
export function ShareButton({ url, title = "Share", className = "" }: Props) {
  ...
  return (
    <button
      onClick={handleShare}
      className={`flex items-center gap-2 px-3 py-1.5 bg-surface-container hover:bg-surface-variant text-on-surface font-medium transition-colors ${className}`}
      title={title}
    >
      {copied ? <Check size={16} className="text-primary-container" /> : <Share2 size={16} />}
